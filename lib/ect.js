/*!
 * ECT CoffeeScript template engine v0.2.8
...
(function () {
	...
	var	ECT = function (options) {
			...
			parse = function (template, callback) {
				...
				for (var i = 0; i < matches.length; i++) {
					...
					if (i % 2 === 1) {
						...
						case '=':
							prefix = '\' + __ectTemplateContext.empty(' + line + ') + __ectTemplateContext.escape(';
							postfix = ') + \'';
							newline = '';
							text = text.substr(1);
							break;
						case '-':
							prefix = '\' + __ectTemplateContext.empty(' + line + ') + __ectTemplateContext.out(';
							postfix = ') + \'';
							newline = '';
							text = text.substr(1);
							break;
						...
						case 'partial' :
							...
							prefix = '\' + __ectTemplateContext.empty(' + line + ') + (';
							postfix = ') + \'';
		...
		TemplateContext.prototype.escape = function (text) {
			...
			return text.replace(/&|<|>|"/g, function (char) { return escapedChars[char]; });
		};

		TemplateContext.prototype.out = function (text) {
			if (typeof text === 'undefined') return '';
			return text;
		};

		TemplateContext.prototype.empty = function () { return ''; };
		...
		TemplateContext.prototype.load = function (template, callback) {
			...
			} else if (ect.options.cache && cache[template]) {
