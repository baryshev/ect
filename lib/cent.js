/*!
 * Cent JavaScript template engine v0.1.0
...
(function () {
	...
	var
		...
		Cent = function (newOptions) {
			if (!(this instanceof Cent)) {
				return new Cent(newOptions);
			}			
			var
				options = {
					open : '<%',
				...
				parse = function (html) {
					var
						...
						length,	i, text, command, line, state,
					...
					for (i = 0, length = matches.length; i < length; i++) {
						...
						if (i % 2 === 1) {
							...
							case '@':
								prefix = '\', 0[' + line + '], (__centTemplate.partial ';
								postfix = '), \'';
								newline = '';
								state = STATE_PARTIAL;
								text = text.substr(1);
								break;
							case '!':
								prefix = '\', 0[' + line + '], (__centTemplate.extend ';
								postfix = '), \'';
								newline = '';
								state = STATE_EXTEND;
								text = text.substr(1);
								break;
							case '*':
								prefix = '\', 0[' + line + '], (__centTemplate.child \'';
								postfix = '\'), \'';
								newline = '';
								text = text.substr(1);
								break;
							case '[':
								prefix = '\'\n' + line + '\n__centTemplate.blockStart \'';
							...
							case STATE_CODE:
								...
								if (indentChar = indentChars[text.charAt(text.length - 1)]) {
									...
									text = text.replace(indentTrim, '').replace(trimExp, '');
									indent = true;
								}
								switch (command) {
								case 'end' :
							...
							case STATE_EXTEND:
								text = text.split(/\s+/);
								if (text.length > 1) {
									text = ['\'' + text[0] + '\'', text.splice(1).join(' ')].join(', ');
								} else {
									text = '\'' + text[0] + '\'';
								}
								buffer.push(prefix, text, postfix);
								break;
							}
						} else {
							if (indentStack[indentStackPointer] !== 'switch') {
								buffer.push(text.replace(/[\\']/g, '\\$&').replace(/\r/g, '').replace(newlineExp, '\\n'));
							}
						}
						lineNo += text.split(/\n/).length - 1;
					}
					...
					buffer = buffer.join('');
					return new Function('__centTemplate', 'return ' + CoffeeScript.compile(buffer));
				},
			...
			Template.prototype.render = function (callback) {
				...
				load(this.file, function (error, blank) {
					if (error) {
						...
						try {
							var buffer = blank.call(that.data, that);
