/*!
 * Cent JavaScript template engine v0.2.2
...
(function () {
	...
	var
		...
		Cent = function (options) {
			if (!(this instanceof Cent)) {
				return new Cent(options);
			}
			var cent = this;
			...
			var
				...
				parse = function (html) {
					var
						lineNo = 1,
						bufferStack = [ '__centTemplate.buffer' ], bufferStackPointer = 0,
						buffer = [ bufferStack[bufferStackPointer] + '.push \'' ],
						indentChar,
						matches = html.split(new RegExp(regExpEscape(cent.options.open) + '((?:.|[\r\n])+?)(?:' + regExpEscape(cent.options.close) + '|$)')),
					...
					for (i = 0, length = matches.length; i < length; i++) {
						...
						command = '';
						if (i % 2 === 1) {
							line = '__centTemplate.line = ' + lineNo;
							switch (text.charAt(0)) {
							case '=':
								prefix = '\', 0[' + line + '], __centTemplate.escape(';
								postfix = '), \'';
								newline = '';
								text = text.substr(1);
								break;
							...
							if ((indentChar = indentChars[text.charAt(text.length - 1)])) {
								...
								if (indentChar === '>') {
									bufferStack.push('__centBuffer' + bufferStackPointer);
									postfix = '\n__centBuffer' + bufferStackPointer + ' = []';
									bufferStackPointer++;
									postfix += '\n' + bufferStack[bufferStackPointer] + '.push \'';
									command = 'function';
								}
					...
					buffer.push('\'\nreturn __centTemplate.buffer');
					buffer = buffer.join('');
					return new Function('__centTemplate', 'partial', 'extend', 'block', 'content', 'return ' + CoffeeScript.compile(buffer));
				},
				...
				read = function (file, callback) {
					if (Object.prototype.toString.call(cent.options.root) === '[object Object]') {
						try {
				...
				load = function (file, callback) {
					if (cent.options.useCache && cache[file]) {
							...
							read(file, function (error, data) {
								if (error) {
									...
									try {
										...
										if (cent.options.useCache) {
											cache[file] = blank;
										}
										loaded(undefined, file, blank);
										if (cent.options.watchForChanges) {
				...
				Template = function (file, data, customData) {
					this.file = file;
					if (Object.prototype.toString.call(cent.options.root) === '[object String]') {
						this.file = path.normalize((cent.options.root.length ? (cent.options.root + '/') : '') + file + cent.options.ext);
					}
					this.data = data;
					if (customData) {
